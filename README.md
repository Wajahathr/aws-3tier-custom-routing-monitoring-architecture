...
## Project Overview
This project demonstrates a production-ready, highly available 3-tier architecture deployed on AWS. The foundational infrastructure includes a custom VPC, five carefully configured Security Groups, and customized route tables for precise traffic routing[cite: 1]. 

A key highlight of this project is the custom-built NAT Instance which acts as a router for private subnets[cite: 1], offering a cost-optimized alternative to managed NAT Gateways. The application layer features a containerized Java Spring Boot application (VProfile) that was cloned, built locally, and pushed to Amazon ECR[cite: 1]. This application is deployed via Launch Templates, managed by an Auto Scaling Group (ASG), and exposed through an Application Load Balancer (ALB)[cite: 2]. The backend relies on a secure, Multi-AZ RDS MySQL database configured with its own subnet group and dedicated security group[cite: 1].

Furthermore, an Event-Driven Serverless Monitoring Pipeline was implemented using CloudWatch alarms attached to the ASG, triggering an SNS topic and a Lambda function to send real-time alerts to a Discord webhook[cite: 28].
...
When a user accesses the website via the ALB DNS, the traffic follows this path[cite: 3]:
1. **Internet Gateway (IGW):** The user's request first hits the VPC's IGW[cite: 4].
2. **Application Load Balancer (ALB):** The IGW forwards the request directly to the ALB residing in the Public Subnets[cite: 5]. The ALB evaluates incoming traffic on Port 80 to determine the routing destination[cite: 6].
3. **Private EC2 / Docker App:** The request is routed to the Docker container running on Port 8080 within the private subnet[cite: 8].
4. **RDS Database:** For login and data retrieval, the Java Tomcat App uses its connection string to request data from the RDS MySQL instance (Port 3306) located in the isolated private subnet[cite: 9].

---

## Security Groups: The Virtual Firewalls
Security was designed using the "Principle of Least Privilege"[cite: 11]. Each architectural layer only accepts communication from strictly required sources[cite: 12].

| Security Group Name | Assigned To | Inbound Rules | Purpose |
| :--- | :--- | :--- | :--- |
| **ALB-SG** | Load Balancer | `HTTP (80)` & `HTTPS (443)` from `0.0.0.0/0` | Allow global internet access to the website[cite: 13]. |
| **NAT-SG** | NAT Instance | `All Traffic` from `10.0.0.0/16` (VPC CIDR)<br>`SSH (22)` from `0.0.0.0/0` | Route internet traffic for private subnets and act as a bastion/jump server for SSH access[cite: 13]. |
| **App-SG** | ASG (EC2 Machines) | `Custom TCP (8080)` from **ALB-SG**<br>`SSH (22)` from **NAT-SG** | Accept traffic ONLY from the Load Balancer, and allow SSH debugging via the NAT instance[cite: 13]. |
| **RDS-SG** | RDS Database | `MySQL (3306)` from **App-SG** | Ensure the database strictly listens to Application Servers. This is the most secure layer[cite: 13]. |

---

## Route Tables: Network Routing Logic
While Security Groups act as guards, Route Tables provide the map for data packet routing[cite: 15].

**A. Public Route Table (Assigned to Public Subnets - ALB & NAT)** [cite: 16]
* **Target: `local` | Destination: `10.0.0.0/16`** -> Enables internal VPC machine-to-machine communication[cite: 17].
* **Target: `igw-xxxx` | Destination: `0.0.0.0/0`** -> Routes all internet-bound traffic out through the Internet Gateway[cite: 18].
